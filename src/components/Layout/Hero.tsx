...
//                 Drop your Mount Choke photo at the path below (e.g. download a
//                 free-license image from Unsplash/Pexels, or use your own photo,
...
//                 {/* Light gradient overlay - just enough to keep the header feeling
//                     unified with the rest of the (dark) site, while letting the
//                     mountain photo read clearly. Legibility for the headline is
//                     handled separately by a dark panel directly behind the text. */}
//                 <div className="absolute inset-0 bg-gradient-to-b from-stone-950/55 via-stone-900/25 to-stone-950/60"></div>

//                 {/* Soft color accents on top of the overlay, unchanged from before */}
...
//                 {/* Dark panel behind the headline only, so the mountain shows
...
//                         </span>
//                     </h1>
//                     <p className="text-stone-100 font-serif text-sm sm:text-lg max-w-3xl mx-auto leading-relaxed">{t.heroSubtitle}</p>
//                 </div>

//                 <div className="flex flex-col sm:flex-row gap-3 justify-center items-center font-mono text-xs max-w-md mx-auto">
//                     <button onClick={() => scrollToSection('map-tabs-nav')} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-6 rounded-2xl flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-900/40 active:scale-99 transition-all cursor-pointer">
//                         {t.exploreBtn} <ChevronRight className="w-4 h-4" />
//                     </button>
//                     <button className="w-full sm:w-auto bg-white/15 hover:bg-white/25 backdrop-blur-sm text-white py-3.5 px-6 rounded-2xl border border-white/25 flex items-center justify-center gap-1.5 hover:scale-102 transition-all cursor-pointer">
//                         {t.prospectusBtn}
//                     </button>
//                 </div>
...
//                     {[
//                         { value: '4,120m+', label: t.altChoke },
//                         { value: language === 'en' ? 'Weekly' : 'ሳምንታዊ', label: t.tueMarket },
//                         { value: language === 'en' ? '5 Years' : '5 ዓመታት', label: t.taxHoliday },
//                         { value: '24/7 AI', label: t.aiConcierge }
//                     ].map((stat, i) => (
//                         <div key={i} className="bg-stone-950/40 backdrop-blur-sm p-4 rounded-2xl border border-white/15">
//                             <span className="block text-xl sm:text-2xl font-bold text-lime-400 leading-none">{stat.value}</span>
//                             <span className="block text-[10px] text-stone-200 mt-1 uppercase tracking-wider">{stat.label}</span>
//                         </div>
//                     ))}
//                 </div>
//             </div>
...
export default function Hero() {
    ...
    // Record one website view when the Hero component loads
    useEffect(() => {
